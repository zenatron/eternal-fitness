 ...
 */
export const parseDateStringToUTC = (dateString?: string | null): Date | undefined => {
  if (!dateString) return undefined;
  try {
    // Ensure the string is in the correct format
    if (!/^\d{4}-\d{2}-\d{2}$/.test(dateString)) {
        console.warn(`Invalid date string format passed to parseDateStringToUTC: ${dateString}`);
        return undefined;
    }
 ...
 */
export const formatUTCDateToLocalDateShort = (dateInput?: Date | string | null): string => {
  if (!dateInput) return '';
  try {
    ...
    return `${year}-${month}-${day}`;
      ...
      console.error(`Error formatting date ${dateInput} to local short format:`, error);
      return '';
  }
};
...
export const formatUTCDateToLocalDateFriendly = (
  ...
  if (!dateInput) return '';
    try {
        ...
        console.error(`Error formatting date ${dateInput} to local friendly format:`, error);
        return 'Invalid Date';
    }
}; 