...
async function findOrCreateExercise(
  ...
  if (!staticData) {
    console.warn(
      `Exercise key ${exerciseIdKey} not found in static data. Cannot create.`,
    );
  ...
  try {
    console.log(
      `Creating exercise from static data: ${exerciseIdKey} (${staticData.name})`,
    );
    ...
    if (
      ...
      console.warn(
        `Race condition: Exercise ${exerciseIdKey} likely created concurrently. Finding again.`,
      );
      ...
      async (tx) => {
        ...
        await tx.workoutTemplate.update({
          ...
          data: {
            ...
            favorite:
              validatedData.favorite !== undefined
                ? validatedData.favorite
                : existingTemplate.favorite,
        ...
        for (const setData of validatedData.sets) {
          ...
          if (!exerciseId) {
            ...
            console.warn(
              `Skipping set creation because exercise ID ${exerciseIdKey} could not be resolved.`,
            );
        ...
        console.log(
          `Created ${setCreatePromises.length} sets for template ${templateId}`,
        );
