...
import { Prisma } from "@prisma/client"; // Re-add this import
...
export async function PUT(
  ...
  console.log(
    "PUT /api/template/[templateId] received request for FULL UPDATE"
  );
  try {
    const { userId } = await auth();
    ...
    console.log(
      `Request Body for template ${templateId} FULL UPDATE:`,
      JSON.stringify(body)
    );
    ...
    if (!name || !sets || !Array.isArray(sets)) {
      console.error(`Invalid full update data for template ${templateId}:`, {
        nameExists: !!name,
        setsExists: !!sets,
      ...
      return new NextResponse(
        JSON.stringify({
          error:
            "Invalid template data: name and sets array required for full update",
        }),
        { status: 400 }
      );
    }
    ...
    if (!existingTemplate) {
      console.error(
        `Full update failed: Template ${templateId} not found or unauthorized`
      );
      return new NextResponse("Template not found", { status: 404 });
    }
    ...
    const updatedTemplate = await prisma.$transaction(async (tx) => {
      ...
      await tx.workoutTemplate.update({
        ...
        data: {
          name,
          favorite:
            favorite !== undefined ? favorite : existingTemplate.favorite,
        },
      });
      ...
      for (const set of sets) {
        if (
          ...
          console.warn(
            `Skipping set creation in template ${templateId} update due to missing/invalid exercises array:`,
            set
          );
          continue;
        }
        ...
        if (existingExercise) {
          ...
          if (exerciseData) {
            console.log(
              `Exercise ${exerciseIdKey} not found during update, attempting to create...`
            );
            try {
              ...
              console.log(
                `Successfully created exercise with ID: ${exerciseIdKey} during update`
              );
              ...
              if (
                ...
                console.warn(
                  `Race condition during update: Exercise ${exerciseIdKey} was likely created concurrently. Finding again.`
                );
                ...
                console.error(
                  `Failed to create exercise ${exerciseIdKey} during update:`,
                  createError
                );
              }
            }
            ...
            console.warn(
              `Exercise key ${exerciseIdKey} from update data not found in static exercises list. Set will be created without exercise link.`
            );
          }
        }
        ...
        try {
          ...
          // Explicitly type caught error
          console.error(
            `Error re-creating set for template ${templateId} during update:`,
            error
          );
        }
      }
    ...
    console.log(
      `Successfully performed full update for template ${templateId}`
    );
    return NextResponse.json(updatedTemplate);
  } catch (error: unknown) {
    // Explicitly type caught error
    console.error(
      `General error in PUT /api/template/[templateId] (${
        (params as any)?.templateId || "unknown ID"
      }):`,
      error
    );
