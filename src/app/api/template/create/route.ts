...
export async function POST(request: Request) {
  try {
    ...
    console.log("Received create template request:", { name, favorite, setsCount: sets?.length });

    if (!name || !sets || !Array.isArray(sets)) {
      return new NextResponse(
        JSON.stringify({ error: 'Invalid request body: name and sets array required' }), 
        { status: 400 }
      )
    }
    ...
    const createdTemplate = await prisma.$transaction(async (tx) => {
      ...
      for (const set of sets) {
        ...
        if (!set.exercises || !Array.isArray(set.exercises) || set.exercises.length === 0) {
          console.warn(`Skipping set creation in template ${template.id} due to missing/invalid exercises array:`, set);
          continue;
        }
        ...
        if (existingExercise) {
          ...
          if (exerciseData) {
            console.log(`Exercise ${exerciseIdKey} not found, attempting to create...`);
            try {
              ...
              console.log(`Successfully created exercise with ID: ${exerciseIdKey}`);
            } catch (createError: unknown) {
              // Handle potential race condition if another request created it concurrently
              if (createError instanceof Prisma.PrismaClientKnownRequestError && createError.code === 'P2002') { 
                 console.warn(`Race condition: Exercise ${exerciseIdKey} was likely created concurrently. Attempting to find again.`);
                 ...
                 if (foundAfterRace) {
                    ...
                    console.error(`Failed to find exercise ${exerciseIdKey} even after race condition check.`, createError);
                 }
              } else {
                 console.error(`Failed to create exercise ${exerciseIdKey}:`, createError);
                 // Decide if we should throw or continue without linking
                 // For now, log error and continue without linking
              }
            }
          } else {
            console.warn(`Exercise key ${exerciseIdKey} from set data not found in static exercises list. Set will be created without exercise link.`);
          }
        }
        ...
        try {
          ...
          console.error(`Error creating set for template ${template.id}:`, error);
      ...
      const updatedTemplate = await tx.workoutTemplate.update({
        ...
        data: { totalVolume },
        include: { // Include sets and exercises in the final return
          sets: {
            include: {
              exercises: true
            }
