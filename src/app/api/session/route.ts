...
export async function POST(request: Request) {
  try {
      ...
      try {
        ...
        if (!existingSession) {
          return new NextResponse(
            JSON.stringify({
              error: "Scheduled session not found or not authorized",
            }),
    ...
    if (!template) {
      return new NextResponse(
        JSON.stringify({ error: "Template not found or not owned by user" }),
        { status: 404 }
      );
    }
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!sessionId) {
      return new NextResponse(JSON.stringify({ error: "Missing session ID" }), {
        status: 400,
      });
    }
