...
function calculateStreak(sessionDates: Date[]): number {
  ...
  const uniqueUTCDates = uniqueUTCDateStrings.map(dateStr => {
...
export async function GET() {
  try {
    ...
    const currentMonthStats = monthlyStats.find((stats) => stats.year === currentYear && stats.month === currentMonth) || null;
    const prevMonthStats = monthlyStats.find((stats) => stats.year === prevYear && stats.month === prevMonth) || null;
    ...
    const volumeChange = prevMonthStats && prevMonthStats.volume > 0 
      ...
      : ((currentMonthStats?.volume || 0) > 0 ? 100 : 0); // Show 100% if current has volume and previous didn't
    ...
    const recentSessions = await prisma.workoutSession.findMany({
      ...
      take: 3,
      include: {
        workoutTemplate: { // Include template name
          select: { name: true }
        }
      }
    });
    ...
    const dashboardData = {
      ...
      recentActivity: recentSessions.map(session => ({
        ...
        details: `Completed ${formatTimeAgo(session.completedAt)} • ${session.totalVolume.toFixed(0)} ${user.useMetric ? 'kg' : 'lbs'} Vol.`, // Example detail
